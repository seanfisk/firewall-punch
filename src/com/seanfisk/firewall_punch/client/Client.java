...
public class Client
{
	public static void main(String[] args)
	{
		...
		int num=0;
		PeerConnection peer=new PeerConnection(); // Hold the connection to the
		// peer
		try
		{
			...
			while(true)
			{
				...
				com=ProtoCommand.class.getEnumConstants()[protoInt]; // Convert
				// to
				// enum
				if(com==ProtoCommand.CLOSE) // If INFO, exit loop to receive
					// info
					break;
				switch (com)
				{
			...
			System.out.println("Server sent close command, server thread exiting.");
		...
		if(num!=0) // Punch if this is the first client, or the client num
			// hasn't been set
			peer.punch();
		...
		{
			msg=cons.readLine();
			if(msg==null||msg.equalsIgnoreCase("q")||msg.equalsIgnoreCase("quit")||msg.equalsIgnoreCase("exit"))
				break;
			peer.sendMsg(msg);
			System.out.print("> ");
		}
